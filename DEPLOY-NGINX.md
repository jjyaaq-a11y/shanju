...
2. **方式 B：本地构建后只上传产物（已不推荐）**  
   这种方式依赖 standalone 产物同步静态资源、媒体目录与运行目录，维护成本更高。当前项目已经统一回到 **源码部署 + `npm run start`** 模式，不建议再走 standalone。

下面以 **方式 A（在服务器上拉 Git 并构建）** 作为当前推荐方案。若你历史上已经在用 standalone，请尽快迁回方式 A，避免静态资源与上传媒体路径错乱。
